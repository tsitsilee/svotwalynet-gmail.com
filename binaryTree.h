...
bool searchtree(BINARY_TREE_TYPE *tree, char s[]);
int treeheight(BINARY_TREE_TYPE tree);
int maximum(int leftside, int rightside);
double avgheight(int leftside, int rightside);
double averageheight(BINARY_TREE_TYPE tree);
int inorderoutput(FILE *fp_out, BINARY_TREE_TYPE tree, int n);
int printtofile(FILE *fp_out, BINARY_TREE_TYPE tree);
int treelength(int lefttreeside, int righttreeside);
